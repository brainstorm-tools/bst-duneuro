        ...
        for j = 1 : V1.dim(2)
            for k = 1 : V1.dim(3)
                if ((mask.anatomy(i,j,k) == index_wm))&(L1.anatomy(i,j,k)>10^-4)&(L2.anatomy(i,j,k)>10^-4)&(L3.anatomy(i,j,k)>10^-5) % the L values are significants >> then zeros
                    ...
                    if(sum(sum(isnan(S),1),2)>0) % the orientation is nor well defined ... the tensor will be remain isotrop and no translation
...
%                         T
                        ...
                        if (mask.anatomy(i,j,k) == 1) % ++++ get the WM volume and compute 
                            N(1) = N(1) + 1;
                            vol(1) = vol(1) + L1.anatomy(i,j,k)*L2.anatomy(i,j,k)*L3.anatomy(i,j,k);
