...
int run(const Dune::ParameterTree& config)
{
  ...
  auto projections = duneuro::ProjectionsReader<double, 3>::read(config.sub("coils"));
  ...
  for (unsigned int i = 0; i < dipoles.size(); ++i) {
    ...
    auto num_transfer = driver->applyMEGTransfer(*transfer, dipoles[i], config.sub("solution"));

    // compute numerical solution direct
    driver->solveEEGForward(dipoles[i], *solution, config.sub("solution"));
    auto num_direct = driver->solveMEGForward(*solution, config.sub("solution"));

    // compute and store error measures
    auto prefix = std::string("dipole_") + std::to_string(i) + ".";
    absoluteErrors.push_back(absolute_error(num_direct, num_transfer));
  ...
  for (unsigned int i = 0; i < absoluteErrors.size(); ++i) {
    if (absoluteErrors[i] > tolerance) {
      std::cerr << "error for source " << i << " is to big: " << absoluteErrors[i] << " > "
                << tolerance << std::endl;
      return -1;
    }
  }
