function hm=bst_plotmesh(node,varargin)
%
...
if(~isempty(elem))
   ...
   if(regexp(selector,'='))
      if(size(node,2)==4)
          [cutpos,cutvalue,facedata, elemid]=qmeshcut(elem,node(:,1:3),node(:,4),selector);  
      elseif(size(node,2)==3)
          [cutpos,cutvalue,facedata, elemid]=qmeshcut(elem,node,node(:,3),selector);
      else
