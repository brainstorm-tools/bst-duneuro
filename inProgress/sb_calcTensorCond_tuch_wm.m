        ...
        for j = 1 : V1.dim(2)
            for k = 1 : V1.dim(3)
                if ((mask.anatomy(i,j,k) == 1)&(L1.anatomy(i,j,k)>10^-4)&(L2.anatomy(i,j,k)>10^-4)&(L3.anatomy(i,j,k)>10^-5))
                    ...
                    % +++++++++ Adapt the diffusion tensor to the conductivity tensor by linear multiplication by the mean factor value of variation mappd to the conductivity value of the white matter
